...
import { getPlotIdFromCoordinates, Plant, PlotInfo } from './utils';
...
export const plant = async (x: number, y: number, type: string, privateKey: string) => {
  ...
  const farm: Contract = getContract(publicRuntimeConfig.C_FARM, ContractTypes.FARM, { isSignerRequired: true, privateKey });
